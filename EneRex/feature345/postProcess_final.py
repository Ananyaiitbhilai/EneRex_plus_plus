...
if __name__ == "__main__":
    ...
    for item in data:
        ...
        for i, sent in enumerate(item["predicted_relations"]):
            ...
            # get the ner for one sentence.
            for ner in item["predicted_ner"][i]:
                NERDict[(ner[0], ner[1])] = ner
            # print(NERDict)

            # get the relations in the sentence
            for relation in sent:
                if relation[4] == "USED-FOR":
                    ...
                    if selectedNER1 and selectedNER2:
                        ...
                        if selectedNER1[2] == "Task" and selectedNER2[2] == "Method":
                            selectedTask.append(
                                " ".join(
                                    totalSents[selectedNER1[0] : selectedNER1[1] + 1]
                                )
                            )
                            selectedMethod.append(
                                " ".join(
                                    totalSents[selectedNER2[0] : selectedNER2[1] + 1]
                                )
                            )
                            found = True
                            hardConstraint += 1
                        ...
                        elif selectedNER1[2] in ["Task", "Method"] and selectedNER2[
                            2
                        ] not in ["Task", "Method"]:
                            # work with selectedNER2[2]
                            for x in sent:
                                if x[4] in ["PART-OF", "FEATURE-OF", "HYPONYM-OF"] and (
                                    (x[0], x[1]) == (selectedNER2[0], selectedNER2[1])
                                    or (x[2], x[3])
                                    == (selectedNER2[0], selectedNER2[1])
                                ):
                                    if (x[0], x[1]) == (
                                        selectedNER2[0],
                                        selectedNER2[1],
                                    ) and (x[2], x[3]) in NERDict:
                                        # search for (x[2], x[3])
                                        nervalue = NERDict[(x[2], x[3])][2]
                                        if (
                                            nervalue == "Task"
                                            and nervalue != selectedNER1[2]
                                        ):
                                            selectedTask.append(
                                                " ".join(totalSents[x[2] : x[3] + 1])
                                            )
                                            selectedMethod.append(
                                                " ".join(
                                                    totalSents[
                                                        selectedNER1[0] : selectedNER1[
                                                            1
                                                        ]
                                                        + 1
                                                    ]
                        ...
                        elif selectedNER1[2] not in ["Task", "Method"] and selectedNER2[
                            2
                        ] in ["Task", "Method"]:
                            # work with selectedNER1[2]
                            for x in sent:
                                if x[4] in ["PART-OF", "FEATURE-OF", "HYPONYM-OF"] and (
                                    (x[0], x[1]) == (selectedNER1[0], selectedNER1[1])
                                    or (x[2], x[3])
                                    == (selectedNER1[0], selectedNER1[1])
                                ):
                                    if (x[0], x[1]) == (
                                        selectedNER1[0],
                                        selectedNER1[1],
                ...
                if relation[4] in ["FEATURE-OF", "COMPARE"]:
                    A = (relation[0], relation[1])
                    B = (relation[2], relation[3])
                    ...
                    if B in NERDict:
                        selectedNER2 = NERDict[B]
                    print(selectedNER1, selectedNER2)
                    selectedContributions.append(" ".join(item["sentences"][i]))
        ...
        # selectedContributions.extend(selectedSents)
        selectedContributions = list(set(selectedContributions))
        contribSents[item["doc_key"]] = selectedContributions
    ...
    with open(
        Path.cwd().joinpath(outpath, "sentences_task_app_method.json"), "w"
    ) as output:
        json.dump(resultSents, output, indent=4, sort_keys=True)

    with open(
        Path.cwd().joinpath(outpath, "sentences_contributions.json"), "w"
    ) as output:
        json.dump(contribSents, output, indent=4, sort_keys=True)
