...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    ner_tot_pred = 0
    ner_ori_cor = 0
    tot_output_results = defaultdict(dict)
    if not args.eval_unidirect:     # eval_unidrect is for ablation study
        ...
        for example_index, pair_dict in sorted(scores.items(), key=lambda x:x[0]):  
            ...
            for item in no_overlap:
                ...
                output_preds.append((m1[0], m1[1], m2[0], m2[1], pred_label))
                #if pred_label in sym_labels:
            ...
            if do_test:
                ...
                tot_output_results[example_index[0]][example_index[1]] = output_preds
...
#                    cor += 1        
    ...
    for l_idx, line in enumerate(f):
        ...
        for n in range(num_sents):
            item = tot_output_results.get(l_idx, {}).get(n, [])
            #item.sort()
            predicted_re.append( item )

        data['predicted_relations'] = predicted_re
